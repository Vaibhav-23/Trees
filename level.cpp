...
void dfs(int curr,int parent,int level,v2d &adj,v1d &ans){
    ans[curr]=level;
    for(int neighbour : adj[curr]){
        if(neighbour!=parent){
            dfs(neighbour,curr,level+1,adj,ans);
        }
    }
}
void dfs2(int curr,int parent,v2d& adj,v1d& level){
    if(parent==-1) level[curr]=0;
    else level[curr]=level[parent] + 1;

    for(int neighbour : adj[curr]){
        if(neighbour!=parent){
            dfs2(neighbour,curr,adj,level);
        }
    }
}
void solve(){
