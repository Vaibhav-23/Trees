...
int n,m;
int dfs(int curr,int parent,int cnt,v2d &adj,v1d &cats){
    //we have to dfs and if in any path we get more than m cats then just return 0 and go till the leaf
    //need to check for continous m cats
    if(cats[curr]) cnt++;
    else cnt=0;
    if(cnt>m) return 0;
    if(curr!=1 && adj[curr].size()==1) return 1;
    int total = 0;
    for(int child : adj[curr]){
        if(child!=parent){
            total += dfs(child,curr,cnt,adj,cats);
        }
    }
    return total;
}
