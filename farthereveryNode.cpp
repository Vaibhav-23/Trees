...
void farthestLeaf(int curr,int parent,v2d &adj,v1d &farthest){
    for(int neighbour : adj[curr]){
        if(neighbour!=parent){
            farthestLeaf(neighbour,curr,adj,farthest);
            farthest[curr]=max(farthest[curr],1+farthest[neighbour]);
        }
    }
}
void dfs(int curr,int parent,v2d &adj,v1d &ans,v1d &farthest){
    int max1=-1,max2=-1;
    for(int neighbour : adj[curr]){
        if(neighbour!=parent){
            dfs(neighbour,curr,adj,ans,farthest);
            if(max1==-1){
                max1=farthest[neighbour];
            }
            else{
                if(max1 < farthest[neighbour]){
                    max2=max1;
                    max1=farthest[neighbour];
                }
                else if(max2 < farthest[neighbour]){
                    max2=farthest[neighbour];
                }
            }
        }
        ans[curr] = max1 + max2 + 2;
    }
}
...
void solve(){
    ...
    //to calculate the farthest away path for every single node we need to take max1 + max2 for every node
    v1d farthest(n+1),ans(n+1),farthest2(n+1);

    farthestLeaf(1,-1,adj,farthest);
    dfs(1,-1,adj,ans,farthest);
