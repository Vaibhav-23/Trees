...
void dfs(int curr,int parent,int &time,v2d &adj,v1d &inTime,v1d &outTime){
    inTime[curr]=++time;
    for(auto neighbour : adj[curr]){
        if(neighbour!=parent){
            dfs(neighbour,curr,time,adj,inTime,outTime);
        }
    }
    outTime[curr]=++time;
}
void solve(){
    ...
    v1d inTime(n+1),outTime(n+1);
    int time=0;
    dfs(1,-1,time,adj,inTime,outTime);
    int q;
    cin>>q;
    for(int i=0;i<q;i++){
        int x,y;
