...
int farthestNode;
void dfs(int curr,int parent,int dist,int &maxDist,v2d &adj){
    dist++;
    for(int neighbour : adj[curr]){
        if(parent!=neighbour){
            ...
            dfs(neighbour,curr,dist,maxDist,adj);
        }
    }
}
void solve(){
    ...
    //for diameter take a random node x and find the node y which is farthest from x and from this node y we have to find the level of farthest node z and this is diameter
    int maxDist=0;
    dfs(1,-1,0,maxDist,adj);
    maxDist=0;
    dfs(farthestNode,-1,0,maxDist,adj);
    print(maxDist);
}
